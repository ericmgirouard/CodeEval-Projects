...
in a few step for almost all of the integers. But there are interesting exceptions. 
...
Your program should accept as its first argument a path to a filename. Each line in this file is one test case.
 Each test case will conta in an integer n < 10,000. Assume each test case will always have an answer and that it
...
public class ReverseAndAdd {
	public static void main(String[] args) throws Throwable{
		...
		int count = 0;
		int n = 0;
		try {  n = Integer.parseInt(line); } catch (NumberFormatException nfe) {}
		
		while (!isPalindrome(n)) {
			n += findReverse(n);
			if (count > 100) break; //mathematic analysis (by CodeEval) proves this is always doable in less than 100 iterations
			count++;
		}//end while loop: n is a palindrome or iterations exceeded 100
		System.out.println(count + " " + n);
	}//end main


	//input n, find its reverse, return
	public static int findReverse(int n) {
			String nString = Integer.toString(n);
			String reverseString = "";
			for (int i = nString.length()-1; i >= 0 ; i--) 	reverseString += nString.charAt(i);
			return Integer.parseInt(reverseString);
		}
	
	//input int n, find its reverse, return true if reverse equals n (Palindrome)
	public static boolean isPalindrome(int n) {
		String nString = Integer.toString(n);
		String reverseString = "";
		for (int i = nString.length()-1; i >= 0 ; i--) 	reverseString += nString.charAt(i);
		return reverseString.equals(nString);
	}
